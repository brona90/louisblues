...
Requires Python 3 and ImageMagick (`magick` or `convert`) to rasterise
the SVG icon. ImageMagick also needs an SVG decode delegate (librsvg);
on Debian/Ubuntu install `librsvg2-bin`. The icon is a required build
...
│       ├── shuffle.py
│       └── routine.py
├── icon.svg                       # built (generated by build.py)
├── apple-touch-icon.png           # built from icon.svg (required product)
